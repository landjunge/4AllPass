...
- **SEO every time** we ship something people land on: `<title>`, `meta name="description"`, Open Graph, GitHub description + topics. Keywords match the wedge (credential access, agents, Zero-Knowledge, local-first, self-hosted) — not “better Bitwarden.” Wire: [`docs/vault-protocol.md`](../../../docs/vault-protocol.md). Placement: [`docs/vault-storage.md`](../../../docs/vault-storage.md).
...
Current recommended order: [`docs/product-maturity.md`](../../../docs/product-maturity.md) **v3**. Reliability before expansion. Autofill V1, Assist, Why, TOTP, `install.sh` (Intel Mac: Befehl → Fenster) are on `main`. Live `github.com/login` fill is opt-in e2e (one form, no submit). Next feelable: a stranger import test. **Do not rewrite crypto.** Do not implement [`docs/architecture/future-architecture.md`](../../../docs/architecture/future-architecture.md). On crypto/vault/access/sync changes, run [`docs/architecture/future-compatibility-check.md`](../../../docs/architecture/future-compatibility-check.md). Agent/n8n is not the first screen. Apple notarization stays paused. No Safari/Windows import, no 500 providers, no launch post, no tag `v0.1.2` until asked. FastAPI never returns secrets. Do not merge Tollgate. Do not auto-publish `docs/launch-posts.md`.
