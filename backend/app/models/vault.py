...
class KeyEnvelope(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("vault_key_version >= 1", name="envelope_vault_key_version_positive"),
        ...
        CheckConstraint(
            ...
            name="device_key_version_only_on_device_envelope",
        ),
        CheckConstraint(
            "device_key_version IS NULL OR device_key_version >= 1",
            name="device_key_version_positive",
        ),
    ...
    #: Vault-Key generation this envelope wraps; part of the AAD.
    vault_key_version: Mapped[int] = mapped_column(Integer, nullable=False)
    #: Device-Key generation wrapping it; set only for device envelopes.
...
class EncryptedEntry(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("vault_key_version >= 1", name="entry_vault_key_version_positive"),
        CheckConstraint(f"octet_length(nonce) = {NONCE_BYTES}", name="nonce_length"),
        CheckConstraint(f"octet_length(tag) = {TAG_BYTES}", name="tag_length"),
    ...
    crypto_version: Mapped[int] = mapped_column(Integer, nullable=False)
    #: Vault-Key generation that sealed this entry; part of the AAD.
    vault_key_version: Mapped[int] = mapped_column(Integer, nullable=False)
    nonce: Mapped[bytes] = mapped_column(BYTEA, nullable=False)
    ciphertext: Mapped[bytes] = mapped_column(BYTEA, nullable=False)
    tag: Mapped[bytes] = mapped_column(BYTEA, nullable=False)
