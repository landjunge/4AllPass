...
Known-answer tests: `npm test` (default) and `npm run test:crypto:heavy` (large Argon2id profiles). Vector JSON is regenerated by `scripts/generate-vectors.mjs` and checked by `scripts/verify-aes-gcm-vectors.mjs` plus `scripts/verify-argon2id-vectors.py`.
