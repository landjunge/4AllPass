...
Nur sichtbare, editierbare Felder: nicht `hidden` (Attribut **oder** `type=hidden`), nicht `aria-hidden="true"`, nicht in `[inert]` oder geschlossenem `<dialog>`, nicht `disabled`/`readonly`, `display !== none`, `visibility !== hidden`, Opacity nicht 0, nicht `pointer-events: none`, Box mindestens 2×2 CSS-Pixel. Honeypots bleiben leer.
