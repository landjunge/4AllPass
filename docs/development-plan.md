...
**Wedge (8 Wochen):** [`eight-week-agent-access.md`](eight-week-agent-access.md) — Agent credential access, 4AllPass eigenständig, Broker lokal, FastAPI ohne Tokens. Tollgate erst danach als Client. Provider-Vision `#65`, SAL `#67`, 4AP-CAP-1 `#70` bleiben die Specs darunter. Clipboard-Ingest `#59` bleibt far later. Die lokale 2-Minuten-Demo ist [`two-minute-demo.md`](two-minute-demo.md). Keine Week-8-Launch-Claims, bevor jemand sie durchgeklickt hat.
