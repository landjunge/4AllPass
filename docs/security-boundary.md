...
storage auth on the local profile **and** that account owns a vault. A
passwordless `local@127.0.0.1` session is **404** (same as missing). A
throwaway `POST /auth/register` without a vault is the same 404. The **server**
profile does not mount these routes. Grants live in page
