...
import { buildDeviceRegistration, unlockVaultKey, type DeviceIdentity } from "./webauthnPrf.ts";
...
test("registers a device and unlocks with the same PRF output", () => {
  ...
  const envelopes = buildDeviceRegistration(id, vaultKey, prfOutput.slice());
  const unlocked = unlockVaultKey(id, envelopes, prfOutput.slice());
...
test("zeroizes the PRF output passed into registration and unlock", () => {
  ...
  buildDeviceRegistration(id, vaultKey, registerPrf);
  assert.deepEqual(registerPrf, new Uint8Array(32));

  const envelopes = buildDeviceRegistration(id, vaultKey, randomBytes(32));
  const unlockPrf = randomBytes(32);
  assert.throws(() => unlockVaultKey(id, envelopes, unlockPrf));
...
test("rejects unlock with a PRF output from a different credential", () => {
  ...
  assert.throws(() => unlockVaultKey(otherId, envelopes, randomBytes(32)));
});
...
test("rejects unlock with a stale/incorrect PRF output for the same credential", () => {
  ...
  assert.throws(() => unlockVaultKey(id, envelopes, randomBytes(32)));
});
