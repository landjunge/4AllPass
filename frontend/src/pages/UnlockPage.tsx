  ...
  return (
    <div className="centered">
      <form className="card auth" onSubmit={submit}>
        <h2>{t({ de: "Tresor gesperrt", en: "Vault locked" })}</h2>
        {deviceUnlockAvailable ? (
          <>
            <button
              type="button"
            ...
            >
              {busy === "device"
                ? t({ de: "Warten auf dieses Gerät…", en: "Waiting…" })
                : t({ de: "Mit diesem Gerät öffnen", en: "Unlock with this device" })}
            </button>
            {mechanism ? (
              <p className="hint" data-testid="unlock-mechanism">
                {t({ de: "Entsperrt über", en: "Unlocked via" })}{" "}
            ...
            {mechanism === "uv_gated_local" ? (
              <p className="hint" data-testid="unlock-rank3-warning">
                {t({
                  de: "Rang 3 ist nur ein Policy-Tor: Face ID / Touch ID gibt den lokalen Wrapping-Key frei. Das ist keine kryptografische Authenticator-Bindung wie PRF.",
                  en: "Rank 3 is a policy gate only: user verification releases a wrapping key stored in this browser. It is not a hardware-bound PRF secret.",
                })}
              </p>
            ) : null}
            <div className="divider">{t({ de: "oder", en: "or" })}</div>
          </>
        ) : null}
        ...
        {useRecovery ? (
          <label>
            <FieldLabel
              text={t({ de: "Recovery-Schlüssel", en: "Recovery key" })}
              tip={t({ de: "Ersatz für das Tresor-Passwort. Nicht das Konto-Passwort.", en: "Replaces the vault password. Not the account password." })}
            />
            <textarea
              value={recovery}
          ...
          <label>
            <FieldLabel
              ...
              tip={t({ de: "Nur du. Ruhemodus sperrt nicht — nur der Knopf Sperren.", en: "Only you. Sleep does not lock — only the Lock button." })}
            />
            <input
              type="password"
        ...
        <button type="submit" disabled={busy !== "none"} data-testid="unlock-submit">
          {busy === "password" ? t({ de: "Einen Moment…", en: "One moment…" }) : t({ de: "Öffnen", en: "Unlock" })}
        </button>
        <button type="button" className="link" onClick={() => setUseRecovery(!useRecovery)}>
          {useRecovery ? t({ de: "Tresor-Passwort", en: "Use the vault password" }) : t({ de: "Recovery-Schlüssel", en: "Use the recovery key" })}
        </button>
        {damaged ? (
        ...
        <p className="hint" data-testid="unlock-hint">
          {deviceUnlockAvailable
            ? t({ de: "Dieses Gerät kann in einem Schritt entsperren. Das Tresor-Passwort gilt weiter. Ohne Passwort oder Recovery-Kit kein Zurück.", en: "This device can unlock in one step. The vault password still works. No password or recovery kit means no way back." })
            : t({ de: "Öffnen = Tresor-Passwort. Ohne Passwort oder Recovery-Kit kein Zurück — niemand setzt es zurück.", en: "Unlock = vault password. No password or recovery kit means no way back." })}
        </p>
      </form>
    </div>
  );
