...
export function wrapVaultKey(opts: WrapVaultKeyOptions): KeyEnvelope {
  ...
  const aad = envelopeAad(opts.vaultId, opts.type, deviceId);
  const box = encrypt(opts.wrappingKey, opts.vaultKey, aad);
  return buildEnvelope(opts, deviceId, box.nonce, box.ciphertext, box.tag);
}
...
export function unwrapVaultKey(
  ...
  const aad = envelopeAad(vaultId, envelope.type, deviceId);
  const vk = decrypt(
    wrappingKey,
    envelope.nonce,
    envelope.ciphertext,
