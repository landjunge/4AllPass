...
function requireGeneration(source: Record<string, unknown>, key: string, label: string): number {
  ...
  if (value < 1) {
    throw new ProtocolError(`${label}.${key} must be an integer >= 1`);
  }
  return value;
}
...
export function decodeDeviceKeyEnvelope(value: unknown): DeviceKeyEnvelope {
  ...
  if (credentialId.length < CREDENTIAL_ID_BYTES_MIN || credentialId.length > CREDENTIAL_ID_BYTES_MAX) {
    throw new ProtocolError(
      `deviceKeyEnvelope.credentialId must be ${CREDENTIAL_ID_BYTES_MIN}–${CREDENTIAL_ID_BYTES_MAX} bytes`,
    );
  }
  return {
    version: CRYPTO_PROTOCOL_VERSION,
...
export function decodeVaultSnapshot(value: unknown): VaultSnapshot {
  ...
  return {
    ...
    revision,
    vaultKeyVersion,
    cryptoProtocolVersion: requireVersion(source, "cryptoProtocolVersion", "snapshot"),
